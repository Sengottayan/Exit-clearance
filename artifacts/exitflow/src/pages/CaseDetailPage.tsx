...
import { Mail, Briefcase, Calendar, Hash, FileText } from "lucide-react";
import { CaseTimeline } from "@/components/cases/CaseTimeline";
// Placeholder imports, will create these components next
...
export default function CaseDetailPage() {
  ...
  // Security: Managers can only see their own cases
  if (isManager && exitCase.managerId !== user?.id) {
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-12">
      <PageHeader 
        title={exitCase.employeeName}
        breadcrumbs={[{ label: "Cases", href: "/cases" }, { label: exitCase.id }]}
        action={
          <div className="flex gap-3">
            ...
            {(isHR || isAdmin) && (
              <Button variant="outline" disabled={exitCase.status !== 'completed'}>
                Issue Documents
              </Button>
            )}
          </div>
      ...
      <div className="flex items-center gap-4 mb-8 p-4 bg-card border rounded-lg">
              ...
              <CardContent className="p-6">
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground">{exitCase.employeeRole} · {exitCase.employeeDept}</p>
                  </div>
                </div>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-y-6 gap-x-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium capitalize">{exitCase.exitReason.replace('_', ' ')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
